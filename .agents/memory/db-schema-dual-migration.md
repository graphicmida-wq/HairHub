...
- prod: `initMysql` → blocco `migrate()` (MySQL 8 non ha `ADD COLUMN IF NOT EXISTS`,
  ...
  ER_DUP_FIELDNAME e **1054** ER_BAD_FIELD_ERROR).
...
- Quando aggiungi una colonna: aggiungila in `createSqliteTables` E nel `migrate()` di
  ...
  fai anche il backfill (`UPDATE ... JSON_ARRAY(old_col)`) e rendi nullable/abbandona la
  vecchia (`MODIFY old_col ... NULL`, non DROP: evita di toccare la FK).
