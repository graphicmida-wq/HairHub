...
export const appointmentsTable = mysqlTable("appointments", {
  ...
  servicePrices: json("service_prices").$type<number[]>(),
  serviceListPrices: json("service_list_prices").$type<number[]>(),
  soldProducts: json("sold_products").$type<{ productId: string; quantity: number; unitPrice: number }[]>(),
